        ...
        <div class="inner-main-body p-2 p-sm-3 collapse forum-content show">
            ...
            <div class="row align-items-start">
                <div class="col-lg-12">
                    ...
                    <div>
                        <input type="button" id="write" class="btn btn-primary" value="글 등록" style="float: right"
                               onclick="location.href='/admin/board/write?kindOfBoard=${param.kindOfBoard}'">
                    </div><br><br>
                        ...
                        <c:forEach var="board" items="${boardForm.content}">
                            ...
                            <tr>
                                ...
                                <c:if test="${param.kindOfBoard == '자유' || param.kindOfBoard == '문의' || param.kindOfBoard == '입양후기'}">
                                    <td class="col-md-1">${board.getbNumber()}</td>
                                    <td class="col-md-3">
                                ...
                                <c:if test="${param.kindOfBoard == 'find'}">
                                    <td class="col-md-1">${board.faNumber}</td>
                                    <td class="col-md-3">
                                ...
                                <c:if test="${param.kindOfBoard == 'look'}">
                                    <td class="col-md-1">${board.laNumber}</td>
                                    <td class="col-md-3">
                                ...
                                <td class="col-md-2">
                                    <c:if test="${param.kindOfBoard == '자유' || param.kindOfBoard == '문의' || param.kindOfBoard == '입양후기'}">
                                        <input type="button" id="modify" class="btn btn-primary" value="수정"
                                               onclick="location.href='/admin/board/modify?kindOfBoard=${param.kindOfBoard}&pk=${board.bNumber}'">
                                        <input type="button" id="delete" class="btn btn-primary" value="삭제" style="background-color: red; border-color: red"
                                               onclick="location.href='/admin/board/delete?kindOfBoard=${param.kindOfBoard}&pk=${board.bNumber}'">
                                    </c:if>
                                    <c:if test="${param.kindOfBoard == 'find'}">
                                        <input type="button" id="modify" class="btn btn-primary" value="수정"
                                               onclick="location.href='/admin/board/modify?kindOfBoard=${param.kindOfBoard}&pk=${board.faNumber}'">
                                        <input type="button" id="delete" class="btn btn-primary" value="삭제" style="background-color: red; border-color: red"
                                               onclick="location.href='/admin/board/delete?kindOfBoard=${param.kindOfBoard}&pk=${board.faNumber}'">
                                    </c:if>
                                    <c:if test="${param.kindOfBoard == 'look'}">
                                        <input type="button" id="modify" class="btn btn-primary" value="수정"
                                               onclick="location.href='/admin/board/modify?kindOfBoard=${param.kindOfBoard}&pk=${board.laNumber}'">
                                        <input type="button" id="delete" class="btn btn-primary" value="삭제" style="background-color: red; border-color: red"
                                               onclick="location.href='/admin/board/delete?kindOfBoard=${param.kindOfBoard}&pk=${board.laNumber}'">
                                    </c:if>
                                </td>
                            </tr>
                            </tbody>
                    ...
                    <ul>
                        <li>
                            <c:if test="${boardForm.startPage > 5}">
                                <a href="${pageContext.request.contextPath}/admin/board?kindOfBoard=${param.kindOfBoard}&pageNo=${boardForm.startPage - 5}">&lt;</a>
                            </c:if>
                        </li>
                        <c:forEach var="pbNum" begin="${boardForm.startPage}" end="${boardForm.endPage}">
                            <c:if test="${boardForm.currentPage == pbNum}">
                                <li class="active">
                                    <a href="${pageContext.request.contextPath}/admin/board?kindOfBoard=${param.kindOfBoard}&pageNo=${pbNum}">${pbNum}</a>
                                </li>
                            </c:if>
                            <c:if test="${boardForm.currentPage != pbNum}">
                                <li>
