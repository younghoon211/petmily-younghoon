	...
	<div class="container">
		<div class="row no-gutters slider-text align-items-end">
			<div class="col-md-9 ftco-animate pb-5">
				<h1 class="mb-0 bread">임보 승인 or 거절 관리</h1>
			</div>
		</div>
	</div>
</section>
						...
						<c:forEach var="temp" items="${temp.content}">
							...
							<th>
								<button type="button" class="btn btn-primary"
										onclick="location.href='/admin/adopt_temp/wait/temp_pet/approve?tNumber=${temp.TNumber}'">승인

								</button>
								&nbsp;
								<button type="button" class="btn btn-primary"
										onclick="location.href='/admin/adopt_temp/wait/temp_pet/refuse?tNumber=${temp.TNumber}'">거절
								</button>
							</th>
							</tbody>
						</c:forEach>
					...
					<ul>
						<li>
							<c:if test="${temp.startPage > 5}">
								<a href="/admin/adopt_temp/wait/temp_pet?pageNo=${temp.startPage - 5}">&lt;</a>
							</c:if>
						</li>
						<li>
						...
						<li class="active">
							<a href="/admin/adopt_temp/wait/temp_pet?pageNo=${pbNum}">${pbNum}</a>
						</li>
						</c:if>
						<c:if test="${temp.currentPage != pbNum}">
							<li>
